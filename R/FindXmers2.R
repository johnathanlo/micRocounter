FindXmers2 <- function(micro_list){
  mon_len = nchar(micro_list$Xmers$Sequence[1])
  xmer <- cbind(micro_list$Xmers$Loci, micro_list$Xmers$Lengths, micro_list$Xmers$Sequence, micro_list$Xmers$SequenceNames)
  ...
  homodimers <- c(strrep("a", mon_len), strrep("g", mon_len), strrep("t", mon_len), strrep("c", mon_len))
  xm.uniq2 <- c()
  for (seq in xm.uniq1)
  {
  ...
  for (seq in xm.uniq)#iterate through list of unique sequences
  {
    ...
    new.levels.pct.genome[i] <- (sum*mon_len)/(micro_list$`Genome Size (Mbp)`*1000000)
